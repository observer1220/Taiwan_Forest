...
let dataList = [];
// 變數select：在HTML檔案裡，選取名為areaId的物件(id)
let select = document.querySelector('.searcharea')
// 變數select：在HTML檔案裡，選取名為TR_Content的物件(class)
let TR_Content = document.querySelector('.TR_Content')

// 監聽變數select，當select改變時，觸發函式getdata
...
function getData() {
    ...
    axios.get('https://recreation.forest.gov.tw/mis/api/BasicInfo/Trail')
        .then(function (response) {
            ...
            // 變數selectArea的內容為areaId.value的值
            let selectArea = areaId.value;
            // 變數str的預設值為空值
            let str = '';
            // 變數len為變數dataList長度的縮寫(給for迴圈使用)
            let len = dataList.length;
            // 從0開始執行for迴圈，當i值小於dataList時，重複執行以下公式
            for (i = 0; i < len; i++) {
                ...
                if (dataList[i].TR_ADMIN == selectArea) {
                    ...
                    var content =
                        '<li>' + '編號' + dataList[i].TRAILID + '：' + dataList[i].TR_CNAME + '<br>' + dataList[i].GUIDE_CONTENT + '</li>';
                    // 變數str的預設值為空值，但當觸發for迴圈裡的選擇器時，str會變成content，並不斷累加
                    str += content;
                    // 在變數TR_Content中插入str字串
